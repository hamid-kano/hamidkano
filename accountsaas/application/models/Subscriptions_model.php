...
class Subscriptions_model extends CI_Model
{
    ...
    {
        $this->db->select('geopos_invoices.id,geopos_invoices.tid,geopos_invoices.invoicedate,geopos_invoices.invoiceduedate,geopos_invoices.total,geopos_invoices.status,geopos_invoices.i_class,geopos_customers.name,geopos_invoices.hash_code as hash_code_main ');
        $this->db->from($this->table);
        $this->db->where($this->table.'.company_id', getCompanyId());
