...
class Customers_model extends CI_Model
{
    ...
    private function _get_datatables_query($id = '')
    {
        ...
        if ($due) {
            ...
            $this->db->where('geopos_customers.company_id', getCompanyId());
    ...
    public function details($custid,$hash_code)
    {
        ...
        $this->db->where('geopos_customers.hash_code', $hash_code);
        $this->db->where('geopos_customers.company_id', getCompanyId());
        if ($this->aauth->get_user()->loc) {
            $this->db->where('geopos_customers.loc', $this->aauth->get_user()->loc);
        } elseif (!BDATA) {
        ...
        if (!$valid['email']) {
            ...
            if ($this->db->insert('geopos_customers', $data)) {
                ...
                $this->aauth->applog("[Client Added] $name ID " . $cid, $this->aauth->get_user()->username);
                    ...
                    $this->aauth->applog("[Client Added] $name ID " . $cid, 'via API');
                    $auto_mail = $this->send_mail_auto($email, $name, $temp_password);
                    $this->load->model('communication_model');
                    $attachmenttrue = false;
                    $attachment = '';
                    $this->communication_model->send_email($email, $name, $auto_mail['subject'], $auto_mail['message'], $attachmenttrue, $attachment);
                    return true;
                }
                echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('ADDED') . $p_string . '&nbsp;<a href="' . base_url('customers/view?id=' . $cid.'&ref='.$hash_code) . '" class="btn btn-info btn-sm"><span class="icon-eye"></span>' . $this->lang->line('View') . '</a>', 'cid' => $cid, 'pass' => $temp_password, 'discount' => amountFormat_general($discount)));
                ...
                if ($othe['other'] && $this->aauth->get_user()) {
                    $auto_mail = $this->send_mail_auto($email, $name, $temp_password);
                    $this->load->model('communication_model');
                    $attachmenttrue = false;
                    $attachment = '';
    ...
    public function edit($id, $name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $customergroup, $taxid, $name_s, $phone_s, $email_s, $address_s, $city_s, $region_s, $country_s, $postbox_s, $docid = '', $custom = '', $language = '', $discount = 0,$hash_code)
    {
        $data = array(
            'name' => $name,
            'company' => $company,
    ...
    public function changepassword($id, $password,$hash_code)
    {
    ...
    public function group_list()
    {
        $whr = "";
        if ($this->aauth->get_user()->loc) {
            $whr = "WHERE (geopos_customers.loc=" . $this->aauth->get_user()->loc . " ) and company_id=".getCompanyId()."";
            if (BDATA) $whr = "WHERE (geopos_customers.loc=" . $this->aauth->get_user()->loc . " OR geopos_customers.loc=0 ) and company_id=".getCompanyId()."";
        } elseif (!BDATA) {
            $whr = "WHERE  geopos_customers.loc=0 and company_id=".getCompanyId()."  ";
        }

        if(empty($whr)){
            $whr = "where company_id=".getCompanyId()." ";
        }

        $query = $this->db->query("SELECT c.*,p.pc FROM geopos_cust_group AS c LEFT JOIN ( SELECT gid,COUNT(gid) AS pc FROM geopos_customers $whr  GROUP BY gid) AS p ON p.gid=c.id and company_id=".getCompanyId()."
    ...
    public function delete($id,$hash_code)
    {
        ...
        if ($this->aauth->get_user()->loc) {
            $this->db->delete('geopos_customers', array('id' => $id, 'loc' => $this->aauth->get_user()->loc,'company_id' =>getCompanyId(),'hash_code'=>$hash_code));

        } elseif (!BDATA) {
            $this->db->delete('geopos_customers', array('id' => $id, 'loc' => 0,'company_id' =>getCompanyId(),'hash_code'=>$hash_code));
        } else {
            $this->db->delete('geopos_customers', array('id' => $id,'company_id' =>getCompanyId(),'hash_code'=>$hash_code));
        }

        if ($this->db->affected_rows()) {
            $this->aauth->applog("[Client Deleted]  ID " . $id, $this->aauth->get_user()->username);
            $this->db->delete('users', array('cid' => $id,'company_id' =>getCompanyId()));
            $this->custom->del_fields($id, 1);
            $this->db->delete('geopos_notes', array('fid' => $id, 'rid' => 1,'company_id' =>getCompanyId()));
            //docs
            ...
            $query = $this->db->get();
            $result = $query->row_array();
            if ($this->db->delete('geopos_documents', array('fid' => $id, 'rid' => 1,'company_id' =>getCompanyId()))) {
    ...
    private function _qto_datatables_query($id, $tyd = 0,$hash_code)
    {
        ...
        $this->db->where('geopos_quotes.company_id', getCompanyId());
        $this->db->where('geopos_customers.hash_code', $hash_code);
    ...
    function qto_datatables($id, $tyd = 0,$hash_code)
    {
        $this->_qto_datatables_query($id,$tyd,$hash_code);
        $this->db->where('company_id', getCompanyId());
        if ($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
    ...
    function qto_count_filtered($id,$tyd=0,$hash_code)
    {
        $this->_qto_datatables_query($id,$tyd,$hash_code);
         $this->db->where('geopos_quotes.company_id', getCompanyId());
        $query = $this->db->get();
    ...
    public function recharge($id, $amount,$hash_code)
    {
    ...
    private function _project_datatables_query($cday = '',$hash_code)
    {
    ...
    function project_datatables($cday = '',$hash_code)
    {


        $this->_project_datatables_query($cday,$hash_code);
        $this->db->where('company_id', getCompanyId());
    ...
    function project_count_filtered($cday = '',$hash_code)
    {
        $this->_project_datatables_query($cday,$hash_code);
        $this->db->where('company_id', getCompanyId());
        $query = $this->db->get();
    ...
    public function project_count_all($cday = '',$hash_code)
    {
        $this->_project_datatables_query($cday,$hash_code);
        $this->db->where('company_id', getCompanyId());
        $query = $this->db->get();
    ...
    private function _notes_datatables_query($id,$hash_code)
    {

        $this->db->from('geopos_notes');
        $this->db->where('fid', $id);
        $this->db->where('geopos_notes.company_id', getCompanyId());
    ...
    function notes_datatables($id,$hash_code)
    {
        $this->_notes_datatables_query($id,$hash_code);
        $this->db->where('company_id', getCompanyId());
        if ($this->input->post('length') != -1)
            $this->db->limit($this->input->post('length'), $this->input->post('start'));
        $query = $this->db->get();
    ...
    function notes_count_filtered($id,$hash_code)
    {
        $this->_notes_datatables_query($id,$hash_code);
        $this->db->where('company_id', getCompanyId());
        $query = $this->db->get();
    ...
    public function notes_count_all($id,$hash_code)
    {
        $this->_notes_datatables_query($id,$hash_code);
        $this->db->where('company_id', getCompanyId());
        $query = $this->db->get();
    ...
    public function editnote($id, $title, $content, $cid,$hash_code)
    {
    ...
    function addnote($title, $content, $cid,$hash_code)
    {
    ...
    function deletenote($id, $cid,$hash_code)
    {
        ...
        return $this->db->delete('geopos_notes', array('id' => $id, 
            'fid' => $cid,
             'rid' => 1,'company_id' => getCompanyId(),'hash_code' => $hash_code
             ));
    ...
    public function documentlist($cid)
    {
        ...
        $this->db->where('fid', $cid);
        $this->db->where('rid', 1);
        $this->db->where('geopos_documents.company_id', getCompanyId());
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    function deletedocument($id, $cid,$hash_code)
    {
        $this->db->select('filename');
        $this->db->from('geopos_documents');
        $this->db->where('id', $id);
    ...
    function document_datatables($cid,$hash_code)
    {
        $this->document_datatables_query($cid,$hash_code);
        $this->db->where('company_id', getCompanyId());
        if ($this->input->post('length') != -1)
            $this->db->limit($this->input->post('length'), $this->input->post('start'));
        $query = $this->db->get();
    ...
    private function document_datatables_query($cid,$hash_code)
    {
    ...
    function document_count_filtered($cid,$hash_code)
    {
        $this->document_datatables_query($cid,$hash_code);
        $this->db->where('company_id', getCompanyId());
        $query = $this->db->get();
    ...
    public function document_count_all($cid,$hash_code)
    {
        $this->document_datatables_query($cid,$hash_code);
        $this->db->where('company_id', getCompanyId());
        $query = $this->db->get();
...
<address>' . $this->config->item('address') . '<br>' . $this->config->item('address2') . '</address>
    ...
    public function get_row_customer($id){
