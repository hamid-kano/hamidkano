...
class Aauth
{
     ...
     */
    public function login($identifier, $pass, $remember = FALSE, $captcha = NULL, $totp_code = NULL)
    {
        ...
        if ($this->config_vars['ddos_protection'] && $captcha && $this->get_login_attempts() > $this->config_vars['recaptcha_login_attempts']) {
            ...
            $this->aauth_db->where('company_id', $this->CI->session->userdata('company_id'));
        ...
        if ($query->num_rows() != 0 && $this->verify_password($password, $row->pass)) {
            ...
            // create session company_id hash_code
            $data = array(
                'id' => $row->id,
    ...
    public function is_loggedin()
    {
        ...
        if($this->CI->session->userdata('loggedin')){
            ...
            $query = $this->aauth_db->where('id', $this->CI->session->userdata('company_id'));
            ...
            if(strtotime($row->end_date) <  strtotime(date('Y-m-d'))){
                ...
                $this->CI->session->sess_destroy();
                 redirect('/user/', 'refresh');
                    ...
                    $data['end_date'] = date('Y-m-d', strtotime(date('Y-m-d'). ' + 7 days'));
                    $data['is_free'] = 1;
                    $this->aauth_db->where('id', $this->CI->session->userdata('company_id'));
                    $this->aauth_db->update("geopos_system", $data);
                }
            }
            //start_date
            //end_date
        ...
        if ($this->CI->session->userdata('loggedin')) {
            ...
            if (!$this->CI->input->cookie('user', TRUE)) {
                ...
                if (!is_numeric($cookie[0]) OR strlen($cookie[1]) < 13) {
                    ...
                    $query = $this->aauth_db->where('company_id', $cookie[4]);
                    $query = $this->aauth_db->where('hash_code', $cookie[5]);
                    $query = $this->aauth_db->get($this->config_vars['users']);
    ...
    public function remind_password($email)
    {
        ...
        $query = $this->aauth_db->where('company_id', $this->CI->session->userdata('company_id'));
        $query = $this->aauth_db->get($this->config_vars['users']);
    ...
    public function update_login_attempts()
    {
        ...
        if ($query->num_rows() == 0) {
            ...
            $data['company_id'] = $this->CI->session->userdata('company_id');
    ...
    public function send_pm($sender_id, $receiver_id, $title, $message)
    {
        ...
        $data = array(
            ...
            'title' => $title,
            'message' => $message,
            'date_sent' => date('Y-m-d H:i:s'),'company_id'=>$this->CI->session->userdata('company_id'),'hash_code'=>getHashCode()
        );
    ...
    public function list_pms($limit = 5, $offset = 0, $receiver_id = NULL, $sender_id = NULL)
    {
        ...
        $this->aauth_db->where('geopos_pms.company_id', $this->CI->session->userdata('company_id'));
    ...
    public function delete_pm($pm_id, $user_id = NULL)
    {
        ...
        if ($user_id == $result->sender_id) {
            if ($result->pm_deleted_receiver == 1) {
                return $this->aauth_db->delete($this->config_vars['pms'], array('id' => $pm_id,'company_id'=>$this->CI->session->userdata('company_id')));
            }
    ...
    public function count_unread_pms($receiver_id = FALSE)
    {
        ...
        //var_dump($this->CI->session->userdata('company_id'));exit;
    ...
    function applog($input1, $input2='')
    {
         $company_id = $this->CI->session->userdata('company_id');
         $hash_code = getHashCode();
         $this->aauth_db->insert('geopos_log', array('note'=>$input1,'user'=>$input2,'company_id'=>$company_id,'hash_code'=>$hash_code));
    }
