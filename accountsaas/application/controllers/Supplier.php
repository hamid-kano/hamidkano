...
class Supplier extends CI_Controller
{
    ...
    public function view()
    {
        ...
        $data['details'] = $this->supplier->details($custid,$hash_code);
    ...
    public function load_list()
    {
        ...
        foreach ($list as $customers) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = '<a href="supplier/view?id=' . $customers->id . '&ref=' . $customers->hash_code . '">' . $customers->name . '</a>';
            $row[] = $customers->address . ',' . $customers->city . ',' . $customers->country;
            $row[] = $customers->email;
            $row[] = $customers->phone;
            $row[] = '<a href="supplier/view?id=' . $customers->id . '&ref=' . $customers->hash_code . '" class="btn btn-info btn-sm"><span class="fa fa-eye"></span> ' . $this->lang->line('View') . '</a> <a href="supplier/edit?id=' . $customers->id . '&ref=' . $customers->hash_code . '" class="btn btn-primary btn-sm"><span class="fa fa-pencil"></span> ' . $this->lang->line('Edit') . '</a> <a href="#" data-object-id="' . $customers->id . '"
                data-object-ref="' . $customers->hash_code . '"  class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
    ...
    public function edit()
    {
        ...
        $data['customer'] = $this->supplier->details($pid,$hash_code);
    ...
    public function editsupplier()
    {
        ...
        if ($id) {
            $this->supplier->edit($id, $name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $taxid,$ref);
        }
    }
    ...
    public function delete_i()
    {
        ...
        if ($this->supplier->delete($id,$hash_code)) {
    ...
    public function inv_list()
    {
        ...
        foreach ($list as $invoices) {
            ...
            $row[] = '<a href="' . base_url("purchase/view?id=$invoices->id&ref=$invoices->hash_code") . '" class="btn btn-success btn-xs"><i class="fa fa-eye"></i> ' . $this->lang->line('View') . '</a> &nbsp; <a href="' . base_url("purchase/printinvoice?id=$invoices->id&ref=$invoices->hash_code") . '&d=1" class="btn btn-info btn-xs"  title="Download"><span class="fa fa-download"></span></a>&nbsp; &nbsp;<a href="#" data-object-ref="' . $invoices->hash_code . '" data-object-id="' . $invoices->id . '"
                data-object-id="' . $invoices->hash_code . '" class="btn btn-danger btn-xs delete-object"><span class="fa fa-trash"></span></a>';
            $data[] = $row;
        }
