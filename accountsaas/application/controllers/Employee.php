...
class Employee extends CI_Controller
{
    ...
    public function view()
    {
        ...
        $data['employee'] = $this->employee->employee_details($id,$hash_code);
    ...
    public function invoices_list()
    {
        ...
        foreach ($list as $invoices) {
            ...
            $row[] = $out;
            $row[] = '<a href="' . base_url("invoices/view?id=$invoices->id&ref=".$invoices->hash_code_inv."") . '" class="btn btn-success btn-xs"><i class="fa fa-eye"></i> View</a> &nbsp; <a href="' . base_url("invoices/printinvoice?id=$invoices->id") . '&d=1" class="btn btn-info btn-xs"  title="Download"><span class="fa fa-download"></span></a>';
    ...
    public function translist()
    {
        ...
        foreach ($list as $prd) {
            ...
            $row[] = $prd->payer;
            $row[] = $prd->method;
            $row[] = '<a href="' . base_url() . 'transactions/view?id=' . $pid . '&ref='.$prd->hash_code.'" class="btn btn-primary btn-xs"><span class="icon-eye"></span> View</a> <a data-object-id="' . $pid . '" class="btn btn-danger btn-xs delete-object"><span class="icon-bin"></span>Delete</a>';
            $data[] = $row;
        }
    ...
    function disable_user()
    {
        ...
        if ($nuid == $uid) {
            ...
            if (isset($result['banned']) and $result['banned'] == 0) {
    ...
    function delete_user()
    {
        ...
        if ($nuid == $uid) {
            ...
            $this->db->delete('geopos_employees', array('id' => $uid,'company_id'=>getCompanyId()));

            $this->db->delete('geopos_users', array('id' => $uid,'company_id'=>getCompanyId()));
    ...
    public function update()
    {
        ...
        $id = $this->input->get('id');
        $hash_code_ref = $this->input->get('ref');
        $this->load->model('employee_model', 'employee');
        if ($this->input->post()) {
            ...
            $this->employee->update_employee($eid, $name, $phone, $phonealt, $address, $city, $region, $country, $postbox, $location, $salary, $department, $commission, $roleid,$hash_code);
            ...
            $data['user'] = $this->employee->employee_details($id,$hash_code_ref);
    ...
    public function updatepassword()
    {
        ...
        if ($this->input->post()) {
            ...
            $head['usernm'] = $this->aauth->get_user()->username;
            $head['title'] = $head['usernm'] . ' Profile';
            $data['user'] = $this->employee->employee_details($id,$hash_code);
            $data['eid'] = intval($id);
            $this->load->view('fixed/header', $head);
    ...
    public function hday_list()
    {
        ...
        foreach ($list as $obj) {
            ...
            $row[] = $day;
            $row[] = $obj->val3;
            $row[] = "<a href='" . base_url("employee/editholiday?id=$obj->id&ref=".$obj->hash_code."") . "' class='btn btn-blue'><i class='fa fa-pencil'></i> " . $this->lang->line('Edit') . "</a> " . '<a href="#" data-object-id="' . $obj->hash_code . '" data-object-id="' . $obj->id . '" class="btn btn-danger delete-object"><span class="fa fa-trash"></span></a>';
    ...
    public function emp_search()
    {
        ...
        if ($name) {
            $query = $this->db->query("SELECT geopos_employees.* ,geopos_users.email FROM geopos_employees  LEFT JOIN geopos_users ON geopos_users.id=geopos_employees.id  WHERE geopos_employees.company_id=".getCompanyId()." $whr (UPPER(geopos_employees.name)  LIKE '%" . strtoupper($name) . "%' OR UPPER(geopos_employees.phone)  LIKE '" . strtoupper($name) . "%') LIMIT 6");
