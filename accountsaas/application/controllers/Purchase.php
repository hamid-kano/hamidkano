...
class Purchase extends CI_Controller
{
    ...
    public function edit()
    {
        ...
        $data['customergrouplist'] = $this->customers->group_list();
        $data['terms'] = $this->purchase->billingterms();
        $data['invoice'] = $this->purchase->purchase_details($tid,$hash_code);
    ...
    $check = $this->stockreturn->check_product_is_company($product_id[$key]);
        if($check["counts"] > 0){            
            ...
            echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('Purchase order success') . "<a href='view?id=$invocieno&ref=$hash_main' class='btn btn-info btn-lg'><span class='fa fa-eye' aria-hidden='true'></span>" . $this->lang->line('View') . " </a>"));
        } else {
    ...
    public function delete_i()
    {
        ...
        if ($this->purchase->purchase_delete($id,$hash_code)) {
    ...
    public function editaction()
    {
        ...
        $this->db->delete('geopos_purchase_items', array('tid' => $invocieno ,'company_id' =>getCompanyId()));
        $product_id = $this->input->post('pid');
        $product_name1 = $this->input->post('product_name', true);
        $product_qty = $this->input->post('product_qty');
     ...
     $check = $this->stockreturn->check_product_is_company($product_id[$key]);
        ...
        if ($flag) {
            ...
            if ($this->db->update('geopos_purchase', $data)) {
                $this->db->insert_batch('geopos_purchase_items', $productlist);
                echo json_encode(array('status' => 'Success', 'message' =>
                    "Purchase order has  been updated successfully! <a href='view?id=$invocieno&ref=$hash_code' class='btn btn-info btn-lg'><span class='fa fa-eye' aria-hidden='true'></span> View </a> "));
            } else {
                echo json_encode(array('status' => 'Error', 'message' =>
