...
class Subscriptions extends CI_Controller
{
    ...
    public function action()
    {
        ...
        if ($ship_taxtype == 'incl') $shipping = floatval($shipping) - floatval($shipping_tax);
        ...
        if ($this->db->insert('geopos_invoices', $data)) {
            ...
            echo json_encode(array('status' => 'Success', 'message' =>
                $this->lang->line('Invoice Success') . " <a href='view?id=$invocieno&ref=".$hash_code."' class='btn btn-blue btn-lg'><span class='fa fa-file-text' aria-hidden='true'></span> " . $this->lang->line('View') . "  </a> &nbsp; &nbsp; <a href='$link' class='btn btn-purple btn-lg'><span class='fa fa-globe' aria-hidden='true'></span> " . $this->lang->line('Public View') . " </a>"));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
                "Invalid Entry!"));
            $transok = false;
        ...
        $data = array('type' => 9, 'rid' => $invocieno, 'col1' => $t_profit, 'd_date' => date('Y-m-d'),'company_id'=>getCompanyId(),'hash_code'=>getHashCode() );
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $invoices) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = '<a href="' . base_url("subscriptions/view?id=$invoices->id&ref=".$invoices->hash_code_main."") . '">' . $invoices->tid . '</a>';
            $row[] = $invoices->name;
            ...
            $row[] = '<a href="' . base_url("subscriptions/view?id=$invoices->id&ref=".$invoices->hash_code_main."") . '" class="btn btn-success btn-sm" title="View"><i class="fa fa-eye"></i></a>&nbsp;<a href="' . base_url("subscriptions/printinvoice?id=$invoices->id&ref=".$invoices->hash_code_main."") . '&d=1" class="btn btn-info btn-sm"  title="Download"><span class="fa fa-download"></span></a>&nbsp;<a href="#" data-object-ref="' . $invoices->hash_code_main . '"   data-object-id="' . $invoices->id . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
            $data[] = $row;
        }
        $output = array(
            "draw" => $_POST['draw'],
    ...
    public function editaction()
    {
        ...
        if ($this->limited) {
            $employee = $this->invocies->invoice_details($iid, $this->limited,$hash_code);
        ...
        if ($this->db->update('geopos_invoices', $data)) {
            ...
            $prodindex = 0;
            $itc = 0;
            $this->db->delete('geopos_invoice_items', array('tid' => $iid,'company_id'=>getCompanyId()));
            $product_id = $this->input->post('pid');
            $product_name1 = $this->input->post('product_name', true);
            $product_qty = $this->input->post('product_qty');
