...
//search product in invoice
    public function search()
    {
        ...
        $qw = '';
        if ($wid > 0) {
            $qw = "(geopos_products.warehouse='$wid') AND geopos_products.company_id=".getCompanyId()." and  ";
        }
        if(empty($qw)){
            $qw = "geopos_products.company_id=".getCompanyId()." and ";
        }
        $join = '';
    ...
    public function csearch()
    {
        ...
        if ($name) {
            $query = $this->db->query("SELECT id,name,address,city,phone,email,discount_c FROM geopos_customers WHERE geopos_customers.company_id=".getCompanyId()." and  $whr (UPPER(name)  LIKE '%" . strtoupper($name) . "%' OR UPPER(phone)  LIKE '" . strtoupper($name) . "%') LIMIT 6");
            $result = $query->result_array();
            echo '<ol>';
            $i = 1;
            foreach ($result as $row) {
    ...
    public function party_search2()
    {
        ...
        if ($name) {
            ...
            foreach ($result as $row) {
                ...
                echo "<li onClick=\"selectCustomer2(0,'" . $row['id'] . "','" . $row['name'] . "',this)\"><span>$i</span><p>".$this->lang->line('Customer')."  ". $row['name'] . " &nbsp; &nbsp  " . $row['phone'] . "</p></li>";
                $i++;
            }
            echo '</ol>';
            ...
            foreach ($result as $row) {
                ...
                echo "<li onClick=\"selectCustomer2(1,'" . $row['id'] . "','" . $row['name'] . "',this)\"><span>$i</span><p>".$this->lang->line('Supplier')."  " . $row['name'] . " &nbsp; &nbsp  " . $row['phone'] . "</p></li>";
                $i++;
            }
            echo '</ol>';
        }
    ...
    public function supplier()
    {
        ...
        if ($this->aauth->get_user()->loc) {
            $whr = ' (loc=' . $this->aauth->get_user()->loc . ' OR loc=0) AND ';
            if (!BDATA) $whr = ' (loc=' . $this->aauth->get_user()->loc . ' ) and company_id='.getCompanyId().' AND ';
        } elseif (!BDATA) {
            $whr = ' (loc=0) and company_id='.getCompanyId().' AND ';
        }
        if(empty($whr)){
            $whr = ' company_id='.getCompanyId().' AND ';
        }
        if ($name) {
            $query = $this->db->query("SELECT id,name,address,city,phone,email FROM geopos_supplier WHERE $whr (UPPER(name)  LIKE '%" . strtoupper($name) . "%' OR UPPER(phone)  LIKE '" . strtoupper($name) . "%') LIMIT 6");
            $result = $query->result_array();
...
//            $barcode = $name;
    ...
    public function v2_pos_search()
    {
        ...
        if (is_numeric($name)) {
            ...
            $query = "SELECT geopos_products.* FROM geopos_products $join WHERE " . $qw . "(UPPER(geopos_products.barcode) LIKE '%" . strtoupper($name) . "%' $bar OR geopos_products.product_code = '" . strtoupper($name) . "') AND (geopos_products.qty>0) ORDER BY geopos_products.product_name  limit 0 , 20";

        }else{
            $query = "SELECT geopos_products.* FROM geopos_products $join WHERE " . $qw . "(UPPER(geopos_products.product_name) LIKE '%" . strtoupper($name) . "%' $bar OR geopos_products.product_code = '" . strtoupper($name) . "') AND (geopos_products.qty>0) ORDER BY geopos_products.product_name  limit 0 , 20  ";
            ...
            $out .= '    <div class="col-3 border mb-1"  ><div class=" rounded" >
                                 <a  id="posp' . $i . '"  class="v2_select_pos_item round"   data-name="' . $row['product_name'] . '"  data-price="' . amountExchange_s($row['product_price'], 0, $this->aauth->get_user()->loc) . '"  data-tax="' . amountFormat_general($row['taxrate']) . '"  data-discount="' . amountFormat_general($row['disrate']) . '" data-pcode="' . $row['product_code'] . '"   data-pid="' . $row['pid'] . '"  data-stock="' . amountFormat_general($row['qty']) . '" data-unit="' . $row['unit'] . '" >
                                        ...
                                        <div class="text-center" style="margin-top: 4px;">
                                            ...
                                            <strong style="white-space: pre-wrap;font-size:19px;">' .substr($row['product_name'],0,20)  . '</strong>
