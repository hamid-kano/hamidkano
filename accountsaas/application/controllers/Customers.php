...
class Customers extends CI_Controller
{
    ...
    public function view()
    {
        ...
        $data['details'] = $this->customers->details($custid,$hash_code);
    if(!empty($data['details']) and isset($data['details'])){
    ...
    public function load_list()
    {
        ...
        if ($this->input->post('due')) {
            foreach ($list as $customers) {
                ...
                $row[] = '<span class="avatar-sm align-baseline"><img class="rounded-circle" src="' . base_url() . 'userfiles/customers/thumbnail/' . $customers->picture . '" ></span> &nbsp;<a href="customers/view?id=' . $customers->id . '&ref_id=' . $customers->hash_code . '">' . $customers->name . '</a>';
                ...
                $row[] = $customers->email;
                $row[] = $customers->phone;
                $row[] = '<a href="customers/view?id=' . $customers->id . '&ref=' . $customers->hash_code . '" class="btn btn-info btn-sm"><span class="fa fa-eye"></span>  ' . $this->lang->line('View') . '</a> <a href="customers/edit?id=' . $customers->id . '&ref=' . $customers->hash_code . '" class="btn btn-primary btn-sm"><span class="fa fa-pencil"></span>  ' . $this->lang->line('Edit') . '</a><a href="'.base_url('status?id='.$customers->id.'$ref='.$customers->hash_code).'"class="btn btn-primary btn-sm">'.$this->lang->line('Customer Status').'</a> <a href="#" data-object-ref="' . $customers->hash_code . '" data-object-id="' . $customers->id . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
                $data[] = $row;
            }
        } else {
            foreach ($list as $customers) {
                ...
                $row = array();
                $row[] = $no . ' <input type="checkbox" name="cust[]" class="checkbox" value="' . $customers->id . '"> ';
                $row[] = '<span class="avatar-sm align-baseline"><img class="rounded-circle" src="' . base_url() . 'userfiles/customers/thumbnail/' . $customers->picture . '" ></span> &nbsp;<a href="customers/view?id=' . $customers->id . '&ref='.$customers->hash_code.'">' . $customers->name . '</a>';
                $row[] = $customers->address . ',' . $customers->city . ',' . $customers->country;
                $row[] = $customers->email;
                $row[] = $customers->phone;
                $row[] = '<a href="customers/view?id=' . $customers->id . '&ref=' . $customers->hash_code . '" class="btn btn-info btn-sm"><span class="fa fa-eye"></span>  ' . $this->lang->line('View') . '</a> <a href="customers/edit?id=' . $customers->id . '&ref=' . $customers->hash_code . '" class="btn btn-primary btn-sm"><span class="fa fa-pencil"></span>  ' . $this->lang->line('Edit') . '</a><a href="'.base_url('status?id='.$customers->id.'&ref=' . $customers->hash_code).'"class="btn btn-primary btn-sm">'.$this->lang->line('Customer Status').'</a> <a href="#" data-object-ref="' . $customers->hash_code . '" data-object-id="' . $customers->id . '" class="btn btn-danger btn-sm delete-object"><span class="fa fa-trash"></span></a>';
    ...
    public function edit()
    {
        ...
        $hash_code = $this->input->get('ref');
        $data['customer'] = $this->customers->details($pid,$hash_code);
    ...
    public function editcustomer()
    {
        ...
        $hash_code = $this->input->post('ref_id');
        $name = $this->input->post('name', true);
        $company = $this->input->post('company', true);
        $phone = $this->input->post('phone', true);
        $email = $this->input->post('email', true);
        ...
        $discount = $this->input->post('discount', true);
        if ($id) {
            $this->customers->edit($id, $name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $customergroup, $taxid, $name_s, $phone_s, $email_s, $address_s, $city_s, $region_s, $country_s, $postbox_s, $docid, $custom, $language, $discount,$hash_code);
        }
    }
    ...
    public function delete_i()
    {
        if ($this->aauth->premission(11)) {
            ...
            if ($id > 1) {
                if ($this->customers->delete($id,$hash_code)) {
    ...
    public function inv_list()
    {
        ...
        foreach ($list as $invoices) {
            ...
            $row[] = '<a href="' . base_url("invoices/view?id=$invoices->id&ref=$invoices->hash_code") . '" class="btn btn-success btn-xs" title="View Invoice"><i class="fa fa-file-text"></i> </a> <a href="' . base_url("invoices/printinvoice?id=$invoices->id&ref=$invoices->hash_code") . '&d=1" class="btn btn-info btn-xs"  title="Download"><span class="fa fa-download"></span></a> <a href="#" data-object-ref="' . $invoices->hash_code . '" data-object-id="' . $invoices->id . '" class="btn btn-danger btn-xs delete-object" title="Delete"><span class="fa fa-trash"></span></a> ';
            $data[] = $row;
        }
        $output = array(
            "draw" => $this->input->post('draw'),
    ...
    public function qto_list()
    {
        ...
        $cid = $this->input->post('cid');
        $tid = intval($this->input->post('tyd'));
        $hash_code = $this->input->post('ref_id');
        $list = $this->customers->qto_datatables($cid,$tid,$hash_code);
        $data = array();
        $no = $this->input->post('start');
        foreach ($list as $invoices) {
            $no++;
            ...
            $row[] = amountExchange($invoices->total, 0, $this->aauth->get_user()->loc);
            $row[] = '<span class="st-' . $invoices->status . '">' . $this->lang->line(ucwords($invoices->status)) . '</span>';
            $row[] = '<a href="' . base_url("quote/view?id=$invoices->id&ref=$invoices->hash_code") . '" class="btn btn-success btn-xs" title="View Invoice"><i class="fa fa-file-text"></i> </a> <a href="' . base_url("quote/printquote?id=$invoices->id&ref=$invoices->hash_code") . '&d=1" class="btn btn-info btn-xs"  title="Download"><span class="fa fa-download"></span></a> <a href="#" data-ref="' . $project->hash_code . '" data-object-id="' . $invoices->id . '" class="btn btn-danger btn-xs delete-object" title="Delete"><span class="fa fa-trash"></span></a> ';
            $data[] = $row;
        }
        $output = array(
            "draw" => $this->input->post('draw'),
            "recordsTotal" => $this->customers->qto_count_all($cid),
            "recordsFiltered" => $this->customers->qto_count_filtered($cid,$tid,$hash_code),
            "data" => $data,
        );
        //output to json format
        echo json_encode($output);
    ...
    public function balance()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->customers->recharge($id, $amount,$hash_code)) {
    ...
    public function prj_list()
    {
        ...
        $list = $this->customers->project_datatables($cid,$hash_code);
        $data = array();
        $no = $this->input->post('start');
        foreach ($list as $project) {
            $no++;
            $name = '<a href="' . base_url() . 'projects/explore?id=' . $project->id . '&ref=' . $project->hash_code . '">' . $project->name . '</a>';
            ...
            $row[] = '<a href="' . base_url() . 'projects/explore?id=' . $project->id . '&ref=' . $project->hash_code . '" class="btn btn-primary btn-sm rounded" data-id="' . $project->id . '" data-ref="' . $project->hash_code . '" data-stat="0"> ' . $this->lang->line('View') . ' </a> <a class="btn btn-info btn-sm" href="' . base_url() . 'projects/edit?id=' . $project->id . '&ref=' . $project->hash_code . '" data-ref="' . $project->hash_code . '" data-object-id="' . $project->id . '"> <i class="fa fa-pencil"></i> </a>&nbsp;<a class="btn btn-danger btn-sm delete-object" href="#" data-object-id="' . $project->id . '" data-ref="' . $project->hash_code . '"> <i class="fa fa-trash"></i> </a>';
        ...
        $output = array(
            "draw" => $this->input->post('draw'),
            "recordsTotal" => $this->customers->project_count_all($cid,$hash_code),
            "recordsFiltered" => $this->customers->project_count_filtered($cid,$hash_code),
            "data" => $data,
        );
        //output to json format
        echo json_encode($output);
    ...
    public function notes_load_list()
    {
        ...
        $cid = $this->input->post('cid');
        $hash_code = $this->input->post('ref');
        $list = $this->customers->notes_datatables($cid,$hash_code);
        $data = array();
        $no = $this->input->post('start');
        foreach ($list as $note) {
            ...
            $row[] = '<a href="editnote?id=' . $note->id . '&cid=' . $note->fid . '&ref=' . $note->hash_code . '" class="btn btn-info btn-sm"><span class="fa fa-eye"></span> ' . $this->lang->line('View') . '</a> <a class="btn btn-danger btn-sm delete-object" href="#" data-object-ref="' . $note->hash_code . '" data-object-id="' . $note->id . '"> <i class="fa fa-trash"></i> </a>';
            $data[] = $row;
        }

        $output = array(
            "draw" => $this->input->post('draw'),
            "recordsTotal" => $this->customers->notes_count_all($cid,$hash_code),
            "recordsFiltered" => $this->customers->notes_count_filtered($cid,$hash_code),
            "data" => $data,
        );
        echo json_encode($output);
    }
    ...
    public function editnote()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->customers->editnote($id, $title, $content, $cid,$hash_code)) {
    ...
    public function addnote()
    {
        ...
        if ($this->input->post('title')) {
            ...
            $content = $this->input->post('content');
            $customer_hash = $this->input->post('ref_id');
            $hash_code = getHashCode();

            if ($this->customers->addnote($title, $content, $cid,$hash_code)) {
                echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('ADDED') . "  <a href='addnote?id=" . $cid . "&ref=".$hash_code."' class='btn btn-indigo btn-lg'><span class='icon-plus-circle' aria-hidden='true'></span>  </a> <a href='notes?id=" . $cid . "&ref=".$customer_hash."' class='btn btn-grey btn-lg'><span class='icon-eye' aria-hidden='true'></span>  </a>"));
            } else {
                echo json_encode(array('status' => 'Error', 'message' => $this->lang->line('ERROR')));
            }
        } else {
    ...
    public function delete_note()
    {
        ...
        if ($this->customers->deletenote($id, $cid,$hash_code)) {
    ...
    function statement()
    {
        ...
        if ($this->input->post()) {
            ...
            $data['details'] = $this->customers->details($data['id'],$data['ref']);
    ...
    public function documents()
    {
        ...
        $this->session->set_userdata("ref", $data['ref']);
    ...
    public function document_load_list()
    {
        $cid = $this->input->post('cid');
        $hash_code = $this->input->post('ref');
        $list = $this->customers->document_datatables($cid,$hash_code);
        $data = array();
        $no = $this->input->post('start');
        foreach ($list as $document) {
            ...
            $row[] = '<a href="' . base_url('userfiles/documents/' . $document->filename) . '" class="btn btn-success btn-xs"><i class="fa fa-file-text"></i> ' . $this->lang->line('View') . '</a> <a class="btn btn-danger btn-xs delete-object" href="#" data-object-ref="' . $document->hash_code . '" data-object-id="' . $document->id . '"> <i class="fa fa-trash"></i> </a>';
        ...
        $output = array(
            "draw" => $this->input->post('draw'),
            "recordsTotal" => $this->customers->document_count_all($cid,$hash_code),
            "recordsFiltered" => $this->customers->document_count_filtered($cid,$hash_code),
            "data" => $data,
        );
        echo json_encode($output);
    }
    ...
    public function adddocument()
    {
        ...
        if ($this->input->post('title')) {
            ...
            if (!$this->upload->do_upload('userfile')) {
                ...
                $data['response'] = 1;
                $data['responsetext'] = 'Document Uploaded Successfully. <a href="documents?id=' . $cid . '&ref='.$hash_code.'"
