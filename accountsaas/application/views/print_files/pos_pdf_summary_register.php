...
<div id='printbox'>
    ...
    <table class="inv_info">
        <?php foreach($register_s as $register) { 
        ?>
        <tr class="contrast">
            <td colspan="4"><?php echo $register['username']; echo' :'; echo date("Y-m-d g:ia", strtotime($register['o_date'])); ?></td>
        </tr>
        <tr style="background-color:#b2f4e5;">
            <td colspan="2"><?php echo $this->lang->line('Cash') ?></td>
            <td colspan="2"> <?php  echo ($register['cash']+$register['cashInvoices']['cashTotal2'][0]['cashTotal']); ?></td>
            </tr>
             <?php
                foreach ($register['cashInvoices'] as $row) {
if($row['tid'])
{
            echo '<tr>
        ...
        echo'</tr>';
         ...
         <tr style="background-color:#b2d4f4;">
            <td colspan="2"><?php echo $this->lang->line('CashAmountTotal') ?></td>
        
            <td colspan="2"> <?php  echo $register['cash']-$register['cashInvoices']['cashTotal'][0]['cashTotal'];?> </td>
                </tr>
         ...
         <?php
                foreach ($register['cardInvoices'] as $row) {
if($row['tid'])
{
            echo '<tr>
  <td colspan="">' . $row['tid'] . '#</td>
            ...
            if($row['amount_cash'] == 0 || $row['amount_cash'] == null){
             echo' <td colspan="">' . $row['total'] . '</td>';
                  ...
                  echo' <td colspan="">' . $row['amount_card'] . '</td>';
             }
        echo'</tr>';
        } }?>
         <tr style="background-color:#b2f4e5;">
            <td colspan="2"><?php echo $this->lang->line('OnAccount') ?></td>
            <td colspan="2"> <?php  echo $register['bank'] ?></td>
            </tr>
             <?php
                foreach ($register['onAccountInvoices'] as $row) {
if($row['tid'])
{
            echo '<tr>
           <td colspan="">' . $row['tid'] . '#</td>
        ...
        <tr>
            <td colspan="2"><?php echo $this->lang->line('Cash') ?></td>
            <td colspan="2"> <?php  echo $sum_cash+$register['cashInvoices']['cashTotal2'][0]['cashTotal']; ?></td>
            </tr>
              <tr>
            <td colspan="2"><?php echo $this->lang->line('Card Swipe') ?></td>
            <td colspan="2"><?php echo $sum_card+$register['cardInvoices']['cardTotal2'][0]['cardTotal'] ?></td>
            </tr>
              <tr>
            <td colspan="2" ><?php echo $this->lang->line('OnAccount') ?></td>
            <td colspan="2"><?php echo $sum_onaccount ?></td>
        </tr>
        <?php } ?>
